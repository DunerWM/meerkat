    ...
    E.plugin(function () {
        ...
        fns.onload || (fns.onload = function (resultText, xhr) {
            ...
            if (!resultJson.success) {
                // 提示错误
                E.warn('上传失败：' + resultJson.message);
                alert(resultJson.message);
            } else {
                var path = resultJson.data.paths[0];
                E.log('上传成功，即将插入编辑区域，结果为：' + path);

                // 将结果插入编辑器
                img = document.createElement('img');
                img.onload = function () {
                    var html = '<img src="' + path + '" alt="' + originalName + '" style="max-width:100%;"/>';
                ...
                img.onerror = function () {
                    E.error('使用返回的结果获取图片，发生错误。请确认以下结果是否正确：' + path);
                    img = null;
                };
                img.src = path;
            }
